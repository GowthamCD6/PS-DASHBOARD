...
exports.assign_mentor = (req,res,next) => {
    try{
      ...
      if(!allowedStatus.includes(status))return createError.BadRequest('Invalid status!');
      ...
      db.query(isfaculty,[faculty_id],(error,result) => {
        ...
        db.query(checksql,[faculty_id, student_id],(error1, result1) => {
            ...
            db.query(assignsql,[status, faculty_id, student_id, status], (error3,result3) => {
...
exports.remove_mentor = (req,res,next) => {
    try{
      ...
      if(!faculty_id || !student_id.trim())return next(createError.BadRequest("required paramenters are missing!"));
