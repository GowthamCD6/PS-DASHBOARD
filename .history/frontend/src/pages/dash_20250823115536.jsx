...
// Import API services with fallbacks
let studentService, useApiCall;
try {
  const services = await import('../services');
  studentService = services.studentService;
  const hooks = await import('../hooks/useApiCall');
  useApiCall = hooks.useApiCall;
} catch (error) {
  console.warn('Failed to import API services, using fallbacks:', error);
  // Fallback implementations
  studentService = {
    getAllUsers: async () => ({ success: false, message: 'Service not available' })
  };
  useApiCall = () => ({
    loading: false,
    error: null,
    data: null,
    executeApiCall: async () => ({ success: false, message: 'API not available' }),
    clearError: () => {}
  });
}
...
const Dash = () => {
  ...
  try {
    ...
    console.error('Failed to initialize API hook:', error);
    apiHook = {
      loading: false,
      error: null,
      data: null,
  ...
  const { loading, error, data: studentsData, executeApiCall, clearError } = apiHook;
  const [students, setStudents] = useState(initialStudents); // Use initial data as fallback
            ...
            <thead style={styles.thead}>
              <tr>
                ...
                >
                  ...
                  >
                    <span style={{ fontWeight: '600', color: '#475569' }}>Cumulative Rewards</span>
                    <IconButton
                      size="small"
                  ...
                  <Popover
                    id={Boolean(cumulativeAnchorEl) ? 'cumulative-rewards-popover' : undefined}
                    ...
                    <Box sx={{ p: 3 }}>
                      ...
                      >
                        ...
                        <Select
                          ...
                          onChange={(e) =>
                            setCumulativePopoverFilter((prev) => ({ ...prev, type: e.target.value }))
                          }
                          sx={{
                            width: '100%',
                            '& .MuiOutlinedInput-root': {
                        ...
                        <TextField
                          ...
                          value={cumulativePopoverFilter.value}
                          onChange={(e) =>
                            setCumulativePopoverFilter((prev) => ({ ...prev, value: e.target.value }))
                          }
                          sx={{
                            width: '100%',
                            '& .MuiOutlinedInput-root': {
