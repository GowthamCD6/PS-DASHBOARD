...
const StudentSkillsTable = () => {
  ...
  const [skillColumns, setSkillColumns] = useState([
    { id: 'skill1', name: 'Skill 1', selectedSkill: 'JavaScript', color: '#1976d2' },
  ...
  const [students, setStudents] = useState([
    {
      ...
      totalLevels: 10,
      completedLevels: 7,
      skillLevels: { skill1: 85, skill2: 78 },
      avatar: '👨‍💻'
    },
    {
      id: 2,
      ...
      totalLevels: 8,
      completedLevels: 6,
      skillLevels: { skill1: 72, skill2: 65 },
      avatar: '👨‍🎓'
    },
    {
      id: 3,
      ...
      totalLevels: 12,
      completedLevels: 9,
      skillLevels: { skill1: 90, skill2: 88 },
      avatar: '👩‍💻'
    },
    {
      id: 4,
      ...
      totalLevels: 15,
      completedLevels: 12,
      skillLevels: { skill1: 95, skill2: 92 },
      avatar: '👩‍🔬'
    }
  ]);
  ...
  const availableSkills = [
    ...
    'PHP', 'Ruby', 'Go', 'Rust', 'TypeScript', 'Swift', 'Kotlin', 'Docker', 'AWS', 'Git'
  ];
  ...
  const stats = useMemo(() => {
    ...
    const avgCompletion = totalStudents > 0 
      ? filteredStudents.reduce((sum, student) => sum + (student.completedLevels / student.totalLevels), 0) / totalStudents
      : 0;
  ...
  const LevelBox = ({ value, onClick, type = 'level', maxValue = 100 }) => {
    ...
    return (
      <Tooltip title={`${value}${type === 'skill' ? '%' : ''} ${type === 'total' ? 'Total' : type === 'completed' ? 'Completed' : 'Skill Level'}`}>
        <Box 
          sx={{
            display: 'inline-flex',
            alignItems: 'center',
            ...
            bgcolor: type === 'total' ? 'primary.50' : 
                    type === 'completed' ? 'success.50' : 
                    type === 'skill' ? 'info.50' : 'grey.100',
            borderColor: type === 'total' ? 'primary.300' : 
                        type === 'completed' ? 'success.300' : 
                        type === 'skill' ? 'info.300' : 'grey.300',
            color: type === 'total' ? 'primary.800' : 
                  type === 'completed' ? 'success.800' : 
                  type === 'skill' ? 'info.800' : 'grey.800',
            '&:hover': {
              transform: 'translateY(-2px)',
              boxShadow: 3,
              borderColor: type === 'total' ? 'primary.500' : 
                          type === 'completed' ? 'success.500' : 
                          type === 'skill' ? 'info.500' : 'grey.500'
            }
          }}
          onClick={onClick}
        >
          {type === 'skill' && (
            <Box
              sx={{
                ...
                height: `${percentage}%`,
                width: '100%',
                bgcolor: value >= 80 ? 'success.200' : value >= 60 ? 'warning.200' : 'error.200',
                opacity: 0.3,
        ...
        <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 3 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              <Card sx={{ minWidth: 120, bgcolor: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)' }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
            ...
            <Button
              ...
              startIcon={isLoading ? <Skeleton width={20} height={20} /> : <AddIcon />}
              variant="contained"
              sx={{ 
                ml: 'auto',
        ...
        <Box sx={{ position: 'relative' }}>
          ...
          <Box sx={{ display: 'flex' }}>
              ...
              <TableContainer>
                <Table sx={{ borderCollapse: 'collapse' }}>
                  <TableHead sx={{ bgcolor: 'grey.50' }}>
                    <TableRow>
                      <TableCell sx={{ fontWeight: 'bold', borderRight: 1, borderColor: 'grey.200', width: 140 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <TrendingUpIcon fontSize="small" color="primary" />
                          Total Levels
                        </Box>
                      </TableCell>
                      <TableCell sx={{ fontWeight: 'bold', borderRight: 1, borderColor: 'grey.200', width: 150 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          <TrendingUpIcon fontSize="small" color="success" />
                          Completed
                        </Box>
                      </TableCell>
                      <TableCell sx={{ fontWeight: 'bold', borderRight: 1, borderColor: 'grey.200', width: 160 }}>
                        Student Name
                      </TableCell>
                      <TableCell sx={{ fontWeight: 'bold', borderRight: 1, borderColor: 'grey.200', width: 120 }}>
                        Reg No
                      </TableCell>
                    ...
                    {showColumnFilters && (
                      <TableRow sx={{ bgcolor: 'grey.25' }}>
                        <TableCell sx={{ borderRight: 1, borderColor: 'grey.200' }}></TableCell>
                        <TableCell sx={{ borderRight: 1, borderColor: 'grey.200' }}></TableCell>
                  ...
                  <TableBody>
                    {filteredStudents.map((student, index) => (
                      ...
                      >
                        ...
                        <TableCell sx={{ borderRight: 1, borderColor: 'grey.200' }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <Typography sx={{ fontSize: '1.2em' }}>{student.avatar}</Typography>
            ...
            <Box sx={{ flexGrow: 1, overflowX: 'auto' }}>
              <TableContainer>
                <Table sx={{ borderCollapse: 'collapse', minWidth: skillColumns.length * 220 }}>
                  <TableHead sx={{ bgcolor: 'grey.50' }}>
                    <TableRow>
                            ...
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                              ...
                              >
                                <Box sx={{ textAlign: 'left', width: '100%' }}>
                                  <Typography variant="subtitle2" sx={{ fontWeight: 'bold', color: skill.color }}>
                                    {skill.name}
                                  </Typography>
                                  {skill.selectedSkill && (
                          ...
                          >
                            {skill.selectedSkill ? (
                              <SkillInput
                                ...
                                onChange={(level) => updateSkillLevel(student.id, skill.id, level)}
